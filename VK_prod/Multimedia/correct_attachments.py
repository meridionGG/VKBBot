...
async def correct_attachments(self, user_id, session_id):
    ...
    await get_url_to_post(self, result, user_id, session_id)

async def get_url_to_post(self, result, user_id, session_id):
    ...
    async with aiovk.TokenSession(
        ...
        try:
            ...
            print("Error:", e)
        await download_image_to_url(self, url, result, user_id, session_id)

async def download_image_to_url(self, url, result, user_id, session_id):
    ...
    for filename in os.listdir(DOWNLOAD_DIR):
        ...
        if os.path.isfile(filepath):  # Проверяем, что это файл, а не папка
            ...
            await save_image_on_vk(self, photo_data, server, hash, result, user_id, session_id)

async def save_image_on_vk(self, photo_data, server, hash, result, user_id, session_id):
    async with aiovk.TokenSession(
            access_token=result) as session:
        vk = aiovk.API(session)
        print(type(server))
